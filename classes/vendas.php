...
class vendas{
	...
	public function criarVenda(){
		...
		// resposta vem inicialmento como 0
		$r=0;
		// verifica o separadores 
	...
	public function criarComprovante(){
		$c= new conectar();
		$conexao=$c->conexao();
		// pegando sempre o ultimo id da venda
		$sql="SELECT id_venda from vendas group by id_venda desc";

		$resul=mysqli_query($conexao,$sql);
		$id=mysqli_fetch_row($resul)[0];
		// se id for vazio ou null ou 0 retorna 1
		if($id=="" or $id==null or $id==0){
			return 1;
		}else{
			return $id + 1;
		}// Gera o id sempre acrescentando +1 para cada venda separada
	}
	...
	public function obterTotal($idvenda){
		...
		while($ver= mysqli_fetch_row($result)){
			// faz o somatorio pelo id da venda 
			$total=$total + $ver[0];
		}
