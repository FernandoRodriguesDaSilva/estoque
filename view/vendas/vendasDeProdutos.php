...
<!-- Traz os valores de procedimentos/vendas/obterDadosProdutos.php -->
		...
		$('#produtoVenda').change(function(){
			...
			$.ajax({
				...
				success:function(r){
					...
					//Atributo da imagem em venda dá pra editar
...
<script type="text/javascript">
	...
	function editarP(dados){
		...
		$.ajax({
			...
			success:function(r){
				...
				$('#tabelaVendasTempLoad').load("vendas/tabelaVendasTemp.php");
				alertify.success("Estoque Atualizado, Boa !!");
			}
		});
	}
	...
	function fecharP(index){
		$.ajax({
			type:"POST",
			data:"nao_finalizado=" + index,
			url:"../procedimentos/vendas/fecharProduto.php",
			success:function(r){
				$('#tabelaVendasTempLoad').load("vendas/tabelaVendasTemp.php");
				alertify.success("Produto de vendas Removido com Sucesso!!");
			}
		});
	}
		...
		$.ajax({
			...
			success:function(r){
				...
				if(r > 0){
					...
					alertify.alert("Venda finalizada com Sucesso!");
				}else if(r==0){
...
<!-- Select que é aplicado como ajax na mesma tela -->
